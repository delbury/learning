...
触发重排的条件：
  ...
  5. 元素内容改变时
  6. 添加或删除可见的DOM 元素时
  7. 查询布局信息时，如 offsetTop、scrollTop、clientTop、getComputedStyle() 等
...
重排优化有如下五种方法：
  ...
  5. 在内存中多次操作节点，完成后再添加到文档中去。例如要异步获取表格数据，渲染到页面。可以先取得数据后在内存中构建整个表格的html片段，再一次性添加到文档中去，而不是循环添加每一行。使用 `document.createDocumentFragment: DocumentFragment`
