...
const compareVersion = function (version1, version2) {
  const arr1 = version1.split('.').map((it) => +it);
  const arr2 = version2.split('.').map((it) => +it);
  ...
  while (p1 < arr1.length || p2 < arr2.length) {
    if (p1 < arr1.length) curVal1 = arr1[p1++];
    else curVal1 = 0;

    if (p2 < arr2.length) curVal2 = arr2[p2++];
    else curVal2 = 0;

    if (curVal1 > curVal2) return 1;
    if (curVal1 < curVal2) return -1;
  }
  return 0;
};
...
logAssert(compareVersion, '1.01', '1.001', 0);
logAssert(compareVersion, '1.2', '1.10', -1);
logAssert(compareVersion, '1.0', '1.0.0.0', 0);
logAssert(compareVersion, '1.0.1', '1', 1);
