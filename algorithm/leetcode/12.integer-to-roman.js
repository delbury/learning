...
const ks = ['M', 'CM', 'D', 'CD', 'C', 'XC', 'L', 'XL', 'X', 'IX', 'V', 'IV', 'I'];
const vs = [1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1];
const intToRoman = function (num) {
  let res = '';
  for (let i = 0; i < ks.length; i++) {
    const count = Math.floor(num / vs[i]);
    if (!count) continue;

    res += ks[i].repeat(count);
    num -= count * vs[i];
  }
  return res;
};
...
// logAssert(intToRoman, 1994, 'MCMXCIV');
logAssert(intToRoman, 3, 'III');
